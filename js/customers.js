 ...
 * customers.js
 *
 * Customer Details + WhatsApp Marketing
 *
 * Features:
 * - Customer management
 ...
 * - Name cleaning
 * - Added By tracking
 * - Interested / Not Interested status
 * - Admin-only customer deletion
 * - WhatsApp campaign creation
 * - WhatsApp campaign editing
 * - WhatsApp campaign deletion
 * - Gender targeting
 * - Automatic customer refresh in existing campaigns
 * - Personalized WhatsApp messages
 * - Sent status tracking
 * - Sent By tracking
 * - Alt + / keyboard shortcut for Add Customer
 */
...
function displayCustomerName(customer) {
  const cleaned = cleanCustomerName(
    customer?.name
  );
...
function genderMatches(
  ...
  return (
    campaignGender === 'all' ||
    normalizeGender(customerGender) ===
      campaignGender
  );
}
...
function friendlyFirestoreError(
  ...
  if (
    ...
    return 'Firebase permissions are blocking this feature. Please make sure the updated Firestore rules are published.';
  }
  ...
  if (
    ...
    return 'Firestore needs an index for this query. Please create the suggested index in Firebase Console.';
  }
...
function renderCustomerTableRows(
  filtered
) {
  const tbody =
    document.getElementById(
...
function renderCustomersTable() {
   ...
   *
   * We only update the table rows while
   * typing.
   *
   * This prevents:
   *
  ...
  if (
    ...
    target.innerHTML = `
      <div
        class="customer-directory-toolbar"
      >
      ...
      <div
        class="data-table-wrap customer-table-wrap"
      >
...
export async function saveCustomer(
  ...
  try {
    ...
    /*
     * Refresh Firestore before duplicate
     * checking.
     */
...
export async function renderWhatsappCampaignDetail() {
   ...
   *
   * If a campaign was created yesterday
   * and a new customer was added today,
   * the new customer will appear.
   */
  try {
 ...
 * when the Customers page is active.
 *
 * The shortcut will NOT fire while
 * the user is typing inside:
 *
 * - input
 * - textarea
 * - select
 ...
 * search box should continue behaving normally.
 */
document.addEventListener(
  ...
  event => {
    ...
    /*
     * Detect whether Customer Details
     * page is currently active.
     *
     * We check several possible IDs so
     * this works with the existing SPA
     * structure.
     */
    const customerPage =
      document.getElementById(
