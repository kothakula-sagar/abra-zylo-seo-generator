...
function renderCustomersTable() {
  ...
  const filtered = !query ? _customers : _customers.filter(customer => {
    const name = String(customer.name || '').toLowerCase();
    const number = String(customer.number || '').toLowerCase();
    const normalized = normalizePhone(customer.number);
    return name.includes(query) || number.includes(query) || (digitsQuery && normalized.includes(digitsQuery));
  });
  ...
  target.innerHTML = `
    <div class="customer-directory-toolbar">
      <div class="customer-search-box">
        ...
        <input type="search" id="customer-universal-search" value="${escapeHtml(_customerSearch)}" placeholder="Search by name or number..." autocomplete="off" oninput="window.Customers.searchCustomers(this.value)">
      </div>
      <span class="customer-result-count">Showing ${filtered.length} of ${_customers.length} customers</span>
    </div>
    <div class="data-table-wrap customer-table-wrap">
      <table class="data-table customers-table">
        ...
        <tbody>
          ${filtered.length ? filtered.map((customer, index) => {
            const status = customerStatus(customer);
            return `<tr>
              <td>${index + 1}</td>
              <td>${formatDate(customer.addedAt)}</td>
              <td><strong>${escapeHtml(customer.name || 'Buddy')}</strong></td>
          ...
          }).join('') : '<tr><td colspan="8" class="customer-no-results">No customers found for this search.</td></tr>'}
        </tbody>
      </table>
    </div>`;
}
...
export async function saveCustomer(event) {
  ...
  try {
    ...
    if (duplicate) {
      showCustomerAlert(alert, `This number is already added for ${duplicate.name || 'an existing customer'}. Please use a different number.`);
      return;
    }
...
function buildPersonalizedMessage(customer) {
  const customerName = customer?.name?.trim() || 'Buddy';
...
export async function openWhatsApp(customerId) {
  ...
  try {
    await FB.setDoc(FB.docRef(`${CAMPAIGN_COLLECTION}/${_activeCampaign.id}/recipients`, customer.id), {
      ...
      customerName: customer.name || 'Buddy',
